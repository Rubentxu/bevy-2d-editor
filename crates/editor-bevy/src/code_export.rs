...
fn emit_header(out: &mut String) {
    ...
    writeln!(out, "// Bevy 0.19 | Generated by Bevy 2D Editor");
    writeln!(
        out,
        "// ═══════════════════════════════════════════════════════════════════════════"
    );
...
fn emit_spawn_scene(out: &mut String, scene: &SceneDocument, schemas: &ComponentSchemaRegistry) {
    ...
    let _ = writeln!(
        out,
        "    let mut ids: std::collections::HashMap<String, Entity> = std::collections::HashMap::new();"
    );
    let _ = writeln!(out);
...
mod tests {
    ...
    fn test_codegen_sprite2d() {
        ...
        assert!(
            result
                .source
                .contains("custom_size: Some(Vec2::splat(100.0))")
        );
    ...
    fn test_codegen_user_struct() {
        ...
        assert!(
            result
                .source
                .contains("PlayerHealth { hp: 150, max_hp: 200 }")
        );
    }
    ...
    fn test_struct_name_for_type_id() {
        ...
        assert_eq!(
            struct_name_for_type_id("game.player_health"),
            "PlayerHealth"
        );
