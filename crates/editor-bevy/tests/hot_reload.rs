...
use editor_bevy::hot_reload_source_wasm;
use editor_bevy::source_files;
...
fn hot_reload_source_wasm_pushes_source_request() {
    ...
    let depth = editor_bevy::hot_reload_bus_depth_for_tests();
    assert_eq!(
        depth, 1,
...
fn process_drains_bus_and_invalidates_cache() {
    ...
    editor_bevy::process_hot_reload_requests();
...
fn asset_request_invalidates_body_cache() {
    ...
    editor_bevy::with_asset_body_cache_mut_for_tests(|cache| {
        cache.insert(
            "x".to_string(),
            editor_bevy::scene_asset::SceneAssetDocument {
                layers: vec![],
                asset_id: "asset-x".to_string(),
                logical_path: "x".to_string(),
                role: editor_bevy::scene_asset::SceneAssetRole::Actor,
                version: 1,
                entities: vec![],
                relationships: vec![],
    ...
    editor_bevy::hot_reload_asset_wasm("x");
    editor_bevy::process_hot_reload_requests();

    // Entry should be gone from ASSET_BODY_CACHE
    let found = editor_bevy::with_asset_body_cache_mut_for_tests(|cache| cache.contains_key("x"));
    assert!(
        !found,
    ...
    assert!(
        editor_bevy::is_dirty_for_tests(),
...
fn force_reload_emits_force_variant() {
    ...
    let cache_len = editor_bevy::with_asset_body_cache_mut_for_tests(|cache| cache.len());
