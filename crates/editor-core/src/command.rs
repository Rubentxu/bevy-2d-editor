...
pub enum Command {
    ...
    /// Update the same field on the same component on multiple entities at
    /// once (v0.82 P2, ADR-0025). Inverse is generated by reusing the
    /// existing `Batch`-of-`SetComponentField` apply path; each entity's
    /// pre-state is captured independently so undo restores divergent
    /// values correctly. Empty `entity_ids` is a validation failure; the
    /// frontend filters out entities that do not own `type_id`, so an
    /// inner sub-command failure here is treated as a programming bug
    /// rather than expected runtime input.
...
pub enum CommandError {
    ...
    /// Invalid argument supplied to a command — e.g. empty entity_ids,
    /// duplicate ids in a multi-entity payload (v0.82 P2, ADR-0025).
