...
fn emit_user_structs(out: &mut String, _scene: &SceneDocument, schemas: &ComponentSchemaRegistry) {
    ...
    let _ = writeln!(out, "// ─── User-defined component structs ───────────────────────────────────────────");
    for schema in &user_schemas {
        let name = struct_name_for_type_id(&schema.type_id);
        let _ = writeln!(out, "#[derive(Component)]");
...
fn emit_plugin_shell(out: &mut String) {
    ...
    let _ = writeln!(out, "pub struct ScenePlugin;");
    let _ = writeln!(out);
    let _ = writeln!(out, "impl Plugin for ScenePlugin {{");
    let _ = writeln!(out, "    fn build(&self, app: &mut App) {{");
    let _ = writeln!(out, "        app.add_systems(Startup, spawn_scene);");
    let _ = writeln!(out, "    }}");
    let _ = writeln!(out, "}}");
    let _ = writeln!(out);
}
...
fn emit_spawn_scene(out: &mut String, scene: &SceneDocument, schemas: &ComponentSchemaRegistry) {
    let _ = writeln!(out, "// ─── Scene spawner ─────────────────────────────────────────────────────────────");
    let _ = writeln!(out, "pub fn spawn_scene(mut commands: Commands) {{");
    let _ = writeln!(out, "    let mut ids: std::collections::HashMap<String, Entity> = std::collections::HashMap::new();");
    let _ = writeln!(out);

    for entity in &scene.entities {
        let stable_id = entity.id.as_str();
        let _ = writeln!(out, "    // Entity: {} ({})", entity.name, stable_id);
        let _ = writeln!(out, "    let id = commands.spawn(").unwrap();
        let _ = writeln!(out, "        (Name::new(\"{}\"),),", entity.name).unwrap();
        let _ = writeln!(out, "    ).id();").unwrap();
        let _ = writeln!(out, "    ids.insert(\"{}\".to_string(), id);", stable_id).unwrap();
        let _ = writeln!(out);
        ...
        for component in &entity.components {
            match component.type_id.as_str() {
                ...
                "editor.Sprite2D" => {
                    ...
                    let _ = writeln!(out, "    commands.entity(id).insert(Sprite {{ color: Color::srgba({}, {}, {}, {}), custom_size: Some(Vec2::splat(100.0)), ..default() }});", r, g, b, a).unwrap();
                    // Anchor as string (not a Bevy component — informational)
                    let _ = writeln!(out, "    // anchor: {}", anchor_str);
                }
                ...
                other => {
                    if other.starts_with("game.") {
                        ...
                        if let Some(schema) = schemas.get(other) {
                            ...
                            let _ = writeln!(out, "    commands.entity(id).insert({} {{ {} }});", struct_name, field_inits.join(", "));
                        }
                    }
    ...
    if has_children {
        let _ = writeln!(out, "    // Parent-child hierarchy");
        for entity in &scene.entities {
