...
fn make_instance_with_overrides(
    ...
    SceneInstance {
        ...
        id_map: vec![(
            LocalId::new("root"),
            StableId::new(format!("{}_root", instance_id)),
        )]
        .into_iter()
        .collect(),
        component_overrides,
...
fn s8_version_bump_triggers_resync() {
    ...
    assert_eq!(
        instance.asset_version_seen, 2,
        "asset_version_seen should update to 2"
    );
...
fn s9_resync_never_silently_deletes_overrides() {
    ...
    let total_after =
        instance.component_overrides.len() + instance.orphaned_component_overrides.len();
    assert_eq!(
...
fn s8_multiple_version_bumps() {
    let mut instance = make_instance_with_overrides("inst_003", "npc_asset", 1, vec![]);
...
fn s9_multiple_orphaned_overrides() {
    let mut instance = SceneInstance {
        ...
        id_map: vec![
            (
                LocalId::new("root"),
                StableId::new("inst_multi_orphan_root"),
            ),
            (
                LocalId::new("entity_a"),
                StableId::new("inst_multi_orphan_entity_a"),
            ),
            (
                LocalId::new("entity_b"),
                StableId::new("inst_multi_orphan_entity_b"),
            ),
        ]
        .into_iter()
        .collect(),
    ...
    assert_eq!(
        instance.orphaned_component_overrides.len(),
        2,
        "Should have 2 orphaned overrides"
    );
    assert_eq!(
        instance.component_overrides.len(),
        1,
        "Should have 1 remaining override"
    );
