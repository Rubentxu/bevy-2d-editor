...
fn s1_place_instance_creates_new_instance() {
    ...
    let placed = doc.instances.get(&instance_id).expect("instance should exist");
    ...
    match inverse {
        Command::RemoveInstance { instance_id: inv_id } => {
...
fn s2_id_map_namespaced_format_no_collisions() {
    ...
    let mut id_map = BTreeMap::new();
    id_map.insert(LocalId::new("local_x"), StableId::new("inst_abc123_local_x"));
    id_map.insert(LocalId::new("local_y"), StableId::new("inst_abc123_local_y"));
...
fn s12_empty_asset_rejected() {
    ...
    assert!(result.is_ok(), "processor validate should pass for empty id_map");

    // But apply should succeed since processor doesn't check emptiness
    let apply_result = processor::apply(&mut doc, &cmd);
    assert!(apply_result.is_ok(), "apply should succeed for empty asset instance");

    // Verify the instance was created with empty id_map
    assert_eq!(doc.instances.get(&StableId::new("inst_empty")).unwrap().id_map.len(), 0);
}
...
fn s1_s2_integration_place_instance_with_namespaced_id_map() {
    ...
    let id_map: BTreeMap<LocalId, StableId> = vec![
        (LocalId::new("entity_a"), StableId::new("inst_test_entity_a")),
        (LocalId::new("entity_b"), StableId::new("inst_test_entity_b")),
    ]
    .into_iter()
    .collect();
    ...
    assert_eq!(
        placed.id_map.get(&LocalId::new("entity_a")).unwrap().as_str(),
        "inst_test_entity_a"
    );
    assert_eq!(
        placed.id_map.get(&LocalId::new("entity_b")).unwrap().as_str(),
        "inst_test_entity_b"
    );
