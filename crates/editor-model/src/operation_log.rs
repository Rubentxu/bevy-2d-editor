...
//!
//! H2.3: moved from `editor_bevy::operation_log` so it can live on
//! `EditorSession.active_scene.operation_log` as part of `SceneSessionState`.
//!
//! To break the dependency on `editor_bevy::processor::apply`, undo/redo now
//! take an `apply: &dyn ApplyCommandFn` callback. The Bevy side implements
//! this trait via a unit struct that delegates to `processor::apply`. Tests
//! use a `TestApply` stub. The callback pattern preserves the re-entrancy
//! contract (apply runs while the document is detached from the log borrow)
//! without leaking Bevy-specific types into editor-model.
//!
//! Doc comments will be backfilled in a follow-up; the type/method shapes
//! are otherwise identical to the prior `editor_bevy::operation_log` API.
#![allow(missing_docs)]

use crate::command::{Command, CommandEnvelope, CommandError, CommandMetadata};
use crate::document::SceneDocument;
use crate::ids::StableId;
use serde::{Deserialize, Serialize};
use thiserror::Error;

/// Callback trait for applying a `Command` to a `SceneDocument`. Decouples
/// `OperationLog` from the Bevy-specific processor at the seam.
///
/// Production impl: `editor_bevy::ProcessorApply` (delegates to `processor::apply`).
/// Test impl: any unit struct that runs the command directly on the document.
pub trait ApplyCommandFn {
    /// Apply `cmd` to `doc` and return its inverse. The Bevy impl also handles
    /// pre-resolution of asset bodies via `ProcessorContext`.
...
mod tests {
    ...
    /// Minimal apply callback for tests. Implements just enough of the
    /// `CreateEntity`/`DeleteEntity` pair so undo/redo can be exercised
    /// without pulling in the full Bevy processor.
    struct TestApply;
    ...
    impl ApplyCommandFn for TestApply {
        fn apply(
            ...
            doc: &mut SceneDocument,
            cmd: &Command,
        ) -> Result<Command, crate::command::CommandError> {
            match cmd {
                ...
                Command::SetComponentField {
                    ...
                    if let Some(entity) = doc.entities.iter_mut().find(|e| e.id == *entity_id) {
                        if let Some(component) =
                            entity.components.iter_mut().find(|c| c.type_id == *type_id)
                        {
                            set_dotted_field(&mut component.values, field_path, value.clone());
                        }
                    }
                    Ok(Command::Noop {})
                }
    ...
    /// Minimal dotted-path setter used by `TestApply` to drive undo/redo of
    /// `SetComponentField` in unit tests without pulling in the full Bevy
    /// processor.
    fn set_dotted_field(root: &mut serde_json::Value, path: &str, value: serde_json::Value) {
    ...
    fn apply_for_test(doc: &mut SceneDocument, cmd: &Command) {
        TestApply.apply(doc, cmd).unwrap();
    }
    ...
    fn test_undo_applies_inverse_and_moves_cursor_back() {
        ...
        log.undo(&mut doc, &TestApply).unwrap();
    ...
    fn test_undo_at_start_returns_error() {
        let mut log = OperationLog::new();
        let mut doc = empty_doc();
        let result = log.undo(&mut doc, &TestApply);
    ...
    fn test_redo_applies_forward_and_moves_cursor_forward() {
        ...
        log.redo(&mut doc, &TestApply).unwrap();
        assert_eq!(log.get_cursor(), 0);
    ...
    fn test_redo_at_end_returns_error() {
        ...
        let (fwd, inv) = create_inverse();
        log.record(&envelope_with(fwd), inv);
        let result = log.redo(&mut doc, &TestApply);
