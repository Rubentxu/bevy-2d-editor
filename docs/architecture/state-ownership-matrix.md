...
| ~~`KEYBOARD_STATE`~~  | editor-bevy | ~~`crates/editor-bevy/src/logic_evaluator.rs:1049`~~ | *retired* — moved to `crates/editor-bevy/src/keyboard_state.rs` and renamed to `KEYBOARD_STATE_FALLBACK` (dual-write fallback for legacy `KeyPressedEvaluator`). Canonical owner is the new Bevy `Resource InputState`, populated by `keyboard_state::update_keyboard_state` each frame (Block H). | logic sensor nodes | transient | `Bevy Resource InputState` (in `crate::keyboard_state`) | RETIRED (H2.5 Block H) |

**Progress (after H2.5 Block H, v0.108.8)**: 9 of 9 retired — **H2.5 COMPLETE**
...
`HOT_RELOAD_BUS`, `PLAY_MODE_REQUEST` in Block G; `KEYBOARD_STATE` in
Block H — all migrated via session-first dual-write with renamed
`*_FALLBACK` thread_locals, except `KEYBOARD_STATE` which migrated to a
...
semantics (3-variant and 2-variant). Block H added the new Bevy
`Resource InputState` (lifetime-bound to the Bevy world, the only
H2.5 cell that targets a Bevy Resource instead of EditorSession).
H2.5 milestone complete — forward work is H2.x / H3.x per
`MASTER_ROADMAP.md`.
