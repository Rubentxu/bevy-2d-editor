...
| Global              | Crate        | Declared at                                  | Writers                                          | Readers                                          | Durability | Target owner                       | Migration PR |
|---------------------|--------------|----------------------------------------------|--------------------------------------------------|--------------------------------------------------|------------|------------------------------------|--------------|
| `SCENE_DOC`         | editor-bevy  | `crates/editor-bevy/src/scene_session.rs:59` | `replace_active_doc`, `replace_with_empty`, `process_undo`, scene loaders | `with_active_doc`, `with_active_doc_mut`, undo/redo processors, preview rebuild | session | `EditorSession.active_scene.doc`   | OPEN         |
| `OPERATION_LOG`     | editor-bevy  | `crates/editor-bevy/src/scene_session.rs:64` | `replace_active_doc`, `replace_with_empty`, command processors | `with_log`, `with_log_mut`, undo/redo processors | session | `EditorSession.active_scene.log`   | OPEN         |
| `DIRTY_FLAG`        | editor-bevy  | `crates/editor-bevy/src/scene_state.rs:16`   | scene command processors, save pipeline           | preview rebuild scheduler, autosave hook         | session    | `EditorSession.active_scene.dirty` | OPEN         |
...
| Global                | Crate        | Declared at                                  | Writers                                          | Readers                                          | Durability | Target owner                       | Migration PR |
|-----------------------|--------------|----------------------------------------------|--------------------------------------------------|--------------------------------------------------|------------|------------------------------------|--------------|
| `SCENE_ASSET_DOC`     | editor-bevy  | `crates/editor-bevy/src/asset_state.rs:32`   | asset command processors, scene-asset authoring  | asset UI, instance authoring, resync workbench    | session    | `EditorSession.asset.active`       | OPEN         |
| `ASSET_OPERATION_LOG` | editor-bevy  | `crates/editor-bevy/src/asset_state.rs:37`   | asset command processors                         | undo/redo for asset commands                     | session    | `EditorSession.asset.log`          | OPEN         |
| `ASSET_BODY_CACHE`    | editor-bevy  | `crates/editor-bevy/src/asset_state.rs:39`   | asset hydration, hot-reload                      | asset preview, export pipelines                  | session    | `EditorSession.asset.body_cache`   | OPEN         |
| `RESYNC_REPORTS`      | editor-bevy  | `crates/editor-bevy/src/asset_state.rs:41`   | resync workbench processor                       | override/resync workbench UI                     | session    | `EditorSession.asset.resync`       | OPEN         |
| `VALIDATION_ISSUES`   | editor-bevy  | `crates/editor-bevy/src/asset_state.rs:43`   | validation refresh                               | validation center, scene command gating         | session    | `EditorSession.asset.validation`   | OPEN         |
...
| `PREVIEW_METRICS`     | editor-bevy  | `crates/editor-bevy/src/preview_inspector.rs:60` | preview frame tick, rebuild tracker          | preview inspector UI, telemetry                  | session    | `EditorSession.preview.metrics`    | OPEN         |
| `PREVIEW_MAPPING`     | editor-bevy  | `crates/editor-bevy/src/preview_inspector.rs:68` | rebuild_preview_world                        | preview inspector UI                             | transient  | `EditorSession.preview.mapping`    | OPEN         |
