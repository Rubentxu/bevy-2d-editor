...
| `import-dialog-wiring` (ImportDialog wiring, Aseprite/LDtk/Tiled importer flow) | ✅ RELEASED — v0.110.5 (2026-09-08), cycle `p-28fce7028ac3c497/import-dialog-wiring`, sequence 360→371 | Closes the CP-5 carry-forward from v0.110.4: wires the orphaned `<ImportDialog />` (complete since v0.93 ADR-0041) to the `import-asset-btn` trigger. Adds `handleImportAssetFile` → `setImportDialogOpen(true)`, `handleImportDialogImported` (dispatches `bevy-2d-editor:asset-imported` CustomEvent for catalog refresh), and `handleShowChangeWorkbench` (calls `window.__setActiveBottomTab("workbench")`). Renders `<ImportDialog />` with all 4 required props (isOpen, onClose, onImported, onShowChangeWorkbench). Side-changes: ImportDialog gets `data-testid="import-dialog"` + `tabIndex={-1}` (a11y focus); BottomDock exposes `window.__setActiveBottomTab` test bridge (parallels `__setEditorMode`). New spec `frontend/tests/import-dialog.spec.ts` covers 4 scenarios (S1 happy path + Escape, S2 success + custom event, S3 conflict routing via bridge, S4 error-phase UX). 8/8 new tests pass; 10/10 a11y-critical-paths.spec.ts pass (no regression); `tsc --noEmit` clean. A-min cycle, 4 files / +345/-16 + 8 SDDK artifacts. Tag `v0.110.5` points at commit `200277c`. Three follow-ups remain (separate cycles): highlight specific `changeSetId` in ChangeWorkbench, `ImportDialog.handleReimport` stub, Rust-side importer plugins. |
| `tutorial-walkthrough` (WelcomeOverlay "Take the tour" guided walkthrough, P1 follow-up) | ✅ RELEASED — v0.110.6 (2026-09-08), cycle `p-28fce7028ac3c497/tutorial-walkthrough`, sequence 372→381 | Replaces the `onTakeTour={() => setEditorMode("asset-authoring")}` no-op with a real 5-card guided walkthrough (Inspect Assets → Find Levels → Open Logic → Tweak Components → Play & Test). New `frontend/src/components/TutorialStepper.tsx` (221 LOC, single-purpose state machine) mounts from `AppShell` via `tourOpen` state; steps 1 + 3 call the existing `__setEditorMode` test bridge to switch mode; step 1 also calls the new `__loadSampleProject("platformer-minimal")` test-bridge stub (returns `{ok:true}` after 100 ms — backend loader is a P3 follow-up). `useWelcomeDismissal()` mutual exclusion preserved (`TutorialStepper` returns `null` while overlay is visible). Positional fix: stepper at `bottom:32px right:16px`, banner at `bottom:32px left:16px`, both `z-index:1100` to avoid overlap. New spec `frontend/tests/tutorial-walkthrough.spec.ts` covers 4 scenarios (S1 take-the-tour-opens-stepper-and-switches-to-asset-mode, S2 next-advances-1→5-with-finish-visible-at-step-5, S3 skip-closes-stepper, S4 finish-closes-stepper) × 2 projects (accessibility + full) = 8 tests, all pass. Test-bridge decision: dropped a recorder-wrapper assertion in favour of observing the real DOM side-effect (`project-asset-browser` mounting) because `bindTestHooks()` in `App.tsx` re-binds `window.__setEditorMode` on every render, clobbering recorders. Regression: 8/8 import-dialog.spec.ts pass, 3/3 ux-welcome.spec.ts @accessibility pass (pre-existing 3/3 @full cohort failures confirmed against base `72d9c4a`, NOT introduced by this cycle — tracked as P3 follow-up). tsc + eslint clean. A-min cycle, 4 files / +431/-1 + 7 SDDK artifacts. Tag `v0.110.6` points at commit `c23d84e`. Three follow-ups remain (separate cycles): wire `__loadSampleProject` to backend OPFS loader, persist "completed tour" so the welcome card greys out, fix `ux-welcome.spec.ts` @full cohort. |
| `tour-completed-persistence` (persist tour completion so WelcomeOverlay greys the button after Finish) | ✅ RELEASED — v0.110.7 (2026-09-08), cycle `p-28fce7028ac3c497/tour-completed-persistence`, sequence 382→387 | Closes v0.110.6 carry-forward #2 (P1 UX follow-up). NEW `frontend/src/services/tour.ts` (51 LOC): `markTourCompleted()` writes `{completed:true}` to `.bevy/tour-flags.json` via `opfsSaveFile`, with `localStorage["bevy-2d-editor:tour-completed"]="1"` fallback. `isTourCompleted()` reads back. `TutorialStepper.handleFinish` now `await markTourCompleted()` before `onClose()` — bound to the Finish button on step 5. Skip path unchanged. `WelcomeOverlay` adds a sync `readTourCompletedSync()` reader (mirrors the OPFS-first pattern of `isWelcomePermanentlyDismissedSync`) and reads `tourCompleted` alongside `dismissed` via `Promise.all` in the existing useEffect. When true, the `welcome-tour-btn` renders in a disabled "✓ Tour already taken" state with `aria-disabled="true"`, `data-tour-completed="true"`, and dashed grey CSS class. NEW `frontend/tests/tour-completed-persistence.spec.ts` (108 LOC, 1 scenario × 2 projects = 2 tests): S1 opens the overlay, takes the tour, advances through 5 steps, clicks Finish, reloads, asserts the button is disabled with the correct attributes. Path-fix: initial implementation passed `.bevy/tour-flags.json` directly to `getFileHandle` (OPFS rejects paths with `/`); corrected to nested lookup `.bevy` directory → `tour-flags.json` file. 2/2 new tests pass; 8/8 tutorial-walkthrough regression pass; tsc + eslint clean. Pre-existing 0/3 ux-welcome.spec.ts @full cohort failures confirmed against base `9546284` (NOT introduced by this cycle; ROADMAP P3 carry-forward). A-min cycle, 5 files / +243/-5 + 7 SDDK artifacts (release-receipt + release-report + merge-receipt + archive-manifest + 2 reports). Tag `v0.110.7` points at cycle commit `3bb63c2`. Two carry-forwards still open (separate cycles): wire `__loadSampleProject` to backend OPFS loader (P3); fix `ux-welcome.spec.ts` @full cohort (P3). |
| `load-sample-real-loader` (wire `__loadSampleProject` test bridge to a real OPFS loader) | ✅ RELEASED — v0.110.8 (2026-09-08), cycle `p-28fce7028ac3c497/load-sample-real-loader`, sequence 389→394 | Closes v0.110.6 carry-forward #1. Materialises the cycle-371 test-bridge stub (`window.__loadSampleProject(id)` returns `{ok:true}` after 100 ms) into a real loader: NEW `frontend/src/services/sampleLoader.ts` (131 LOC) exports `OPFS_FILES` (9 entries, single source of truth) + `mountPlatformerMinimal()` that fetches each file from `/examples/platformer-minimal/<localPath>` via the Vite dev-server and writes it to OPFS at the canonical engine-readable path via the existing `window.opfs_save_file` bridge. Best-effort: per-file try/catch, errors aggregated into `errors[]`, never throws. `TutorialStepper` bridge body replaced (cycle-371 stub gone) — calls `mountPlatformerMinimal()` then `window.load_project()`. Test helper NEW `frontend/tests/helpers/sample-loader.ts` (59 LOC) imports `OPFS_FILES` from the production module and reads from disk via `node:fs/promises`, dedup'ing the inlined `OPFS_FILES` + `mountSample` previously duplicated across `e2e-game-creation.spec.ts` and `git-friendly-roundtrip.spec.ts` (diff: -47 / -57 lines per file). NEW `frontend/tests/load-sample-real-loader.spec.ts` (200 LOC, @full cohort, 2 tests): S1.1 production loader writes 9 canonical files to OPFS + parses `project.json` (`{name:"platformer-minimal", scenes:["main"], schemas:["game.PlayerController","game.EnemyPatrol"], scene_assets.length===4}`); S1.2 mirrors the stepper bridge body, hydrates the engine, asserts `list_scene_assets` returns 4 entries with the canonical paths and `list_schemas` includes both custom schemas. Tests: 14/14 in-scope pass (2 new + 8 tutorial-walkthrough + 2 tour-completed + 2 e2e-game-creation); 2 pre-existing `git-friendly-roundtrip.spec.ts` failures verified pre-existing at base `47117df` via `git stash` (NOT introduced by this cycle — separate G2 round-trip issue, ROADMAP P2 carry-forward). `tsc --noEmit` + `eslint --max-warnings=0` clean. A-lite cycle, 6 files / +575/-146 + 7 SDDK artifacts. Tag `v0.110.8` points at cycle commit `0f0564f`. Carry-forwards: fix `git-friendly-roundtrip.spec.ts` G2 round-trip (P2); fix `ux-welcome.spec.ts` @full cohort (P3); production-build sample fallback (out of cycle scope). |
| `fix-git-friendly-roundtrip` (close P2 carry-forward: 2 pre-existing `git-friendly-roundtrip.spec.ts` failures) | ✅ RELEASED — v0.110.9 (2026-09-08), cycle `p-28fce7028ac3c497/fix-git-friendly-roundtrip`, sequence 397→402 | Closes v0.110.8 carry-forward #1 (P2 G2 round-trip issue). (1) NEW test bridge `window.__rehydrateProjectStore` (calls new `rehydrate_project_store` WASM export that re-reads OPFS into the in-memory mirror): MOD `crates/editor-wasm/src/lib.rs` adds `OPFS_STORE: OnceLock<Arc<OpfsProjectStore>>` + `#[wasm_bindgen] pub async fn rehydrate_project_store()`. The composition root keeps the concrete `Arc<OpfsProjectStore>` reference (coerced to `Arc<dyn ProjectStore>` at session-build site) so the rehydrate bridge can call `OpfsProjectStore::hydrate()` on demand. MOD `frontend/src/engine-bridge.ts:151-156` exposes the bridge AFTER `init_project_store()` (parallels `__setEditorMode` / `__loadSampleProject` ordering convention). MOD `frontend/tests/git-friendly-roundtrip.spec.ts:38-40` calls the bridge between `mountSampleInOpfs(page)` and `load_project()`. (2) MOD `examples/platformer-minimal/schemas/game.PlayerController.schema.json` + `examples/platformer-minimal/schemas/game.EnemyPatrol.schema.json`: add top-level `"version": "0.1"` (ADR-0045 — every persisted document MUST declare its format version). (3) **Housekeeping required to verify the cycle** — 5 pre-existing wasm-build blockers (had been masked for ~24 hours by a stale `frontend/src/wasm/` artifact, last rebuild 2026-09-07 08:54 predating the breaking H1.3+H1.4 commit at 2026-09-07 15:51): MOD `crates/editor-bevy/src/lib.rs:2001` E0382 borrow-of-moved-value fix at `discard_scene_changes`; MOD `crates/editor-bevy/src/lib.rs:2102` E0063 missing `extension_data` field at the `SceneDocument` fallback initializer in `load_project`; MOD `crates/editor-bevy/src/wasm_auto_layer.rs:98` lifetime fix in `with_asset_doc` (use `.map(|d| d.clone())` instead of `.cloned()` — `SceneAssetDocument` is Clone but not Copy); MOD `crates/editor-wasm/src/lib.rs:14,368,376` rename `PLAY_MODE_REQUEST` → `PLAY_MODE_REQUEST_FALLBACK` (matches upstream rename in `editor-bevy/src/hot_reload_state.rs:36`); NEW `ImporterRegistry::attach_importer_for_id` method + `ImporterRegistryPort::attach_importer_for_id` trait method + `compose_builtin_importers` uses `attach_importer_for_id` when descriptor pre-seeded by `with_builtins`, fixing the `builtin.aseprite register failed: duplicate importer id` runtime panic. (4) **Bevy↔JS interleaving race fix** (also pre-existing, surfaced during cycle verification): MOD `crates/editor-model/src/ports.rs:182-208` replaces `with_session_mut`'s `arc.lock()` with bounded `try_lock` spin-loop to avoid recursive-mutex panic when Bevy systems hold the SESSION lock during a JS-driven `load_project` call (wasm `no_threads` mutex impl panics on recursive acquisition). Cleaner architectural fix (Bevy pause flag / `RefCell<EditorSession>` on wasm / message-passing runner) tracked for a future cycle. Tests: 2/2 `git-friendly-roundtrip.spec.ts` pass (was: 2/2 failing pre-existing); 2/2 `load-sample-real-loader.spec.ts` regression pass; 871/871 rust unit tests pass. `tsc --noEmit` + `eslint --max-warnings=0` clean. A-min cycle, 9 files / `+204/-17` + 6 SDDK artifacts (explore-report, specification, design, implementation-receipt, verify-report+findings, release-report+receipt+merge-receipt, archive-manifest). Tag `v0.110.9` points at cycle commit `a6262da` (will be moved to archive commit during archive phase per repo's tag-at-cycle-archive pattern). Pre-existing 5/3 smoke failures (`app-characterization` ×4 + `editor-ready` S2) verified pre-existing at base `01b0e50` via `git stash` (NOT introduced — ROADMAP P3 carry-forward). |
| `fix-ux-welcome-full-cohort` (close P3 carry-forward: 3 pre-existing `ux-welcome.spec.ts` `@full` cohort failures) | ✅ RELEASED — v0.110.10 (2026-09-08), cycle `p-28fce7028ac3c497/fix-ux-welcome-full-cohort`, sequence 404→409 | Closes v0.110.6 carry-forward #3 (P3 ux-welcome.spec.ts `@full` cohort — 3 tests, was 0/3 passing pre-existing since `2e0e5ec` "P3 Wave B cohort tagging", Aug 19). The 2e0e5ec sweep correctly added `?skip-welcome=1` to 60 specs that assert against non-welcome surfaces (so the overlay doesn't block pointer events during WASM init), but the same pattern was applied blindly to `ux-welcome.spec.ts` itself, which DOES want the overlay to render. The original `beforeEach`'s intent was: (1) `goto /?skip-welcome=1` to skip the overlay during init; (2) clear OPFS so the overlay treats this as first visit; (3) navigate back to `/` so the WelcomeOverlay re-reads OPFS with the cleared flag and renders. The bug was using `page.reload()` for step 3 — `reload()` preserves the current URL (including `?skip-welcome=1`), so the WelcomeOverlay's synchronous `urlSkip` reader (`WelcomeOverlay.tsx:175-180`) reads `true` and the render guard (`WelcomeOverlay.tsx:228`) short-circuits to `null`. The fix: replace `page.reload()` with `page.goto("/")` so the post-init navigation actually drops the query parameter. 3 inline comments document the 3-phase flow and warn future maintainers about the URL-preservation trap. MOD `frontend/tests/ux-welcome.spec.ts:46-61` ONLY — zero production source change, zero Rust change, zero config change, zero test scenarios added or removed (the 3 existing scenarios share the modified `beforeEach`). Tests: 3/3 `ux-welcome.spec.ts` `@full` cohort pass (was: 0/3 pre-existing); 5/5 regression (`tutorial-walkthrough.spec.ts` + `tour-completed-persistence.spec.ts`) pass; 871/871 rust unit tests pass. `tsc --noEmit` + `eslint --max-warnings=0` clean. A-min cycle, 1 file / `+10/-3` + 6 SDDK artifacts (explore-report, specification, implementation-receipt, verify-report+findings, release-report+receipt+merge-receipt, archive-manifest). Tag `v0.110.10` points at cycle commit `3df1682` (will be moved to archive commit during archive phase per repo's tag-at-cycle-archive pattern). Pre-existing 5 P3 smoke failures (`app-characterization` ×4 + `editor-ready` S2) verified pre-existing at base `a72104f` (NOT introduced — ROADMAP P3 carry-forward, unchanged from v0.110.9). |
