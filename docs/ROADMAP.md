...
*Last updated: v0.72.0 — 2026-07-19 (Hito 4 Order 6 code-aware-ai COMPLETE (3 PRs: #83 v0.70.0 + #84 v0.71.0 + #85 v0.72.0): backend ContextSource trait + 6 impls + ContextBuilder orchestrator + TokenBudget + ProposeRequest extended with 4 optional fields + CreateSourceFile/WriteSourceFile commands + FORBIDDEN_AI_COMMANDS server-side filter (D2 security) + frontend ai-context orchestrator with priority-based greedy fill + ContextDebugSection UI component with per-source token counts + budget meter + per-source toggles + 4 new E2E tests + ADR-0015 + CONTEXT.md updated; 52 ai-proxy + 417 editor-core tests passing; known pre-existing infra issue: 4+4 E2E tests blocked by Vite optimizeDeps bundle-cache race — fetch confirmed working in manual debug, fix requires Vite config investigation; next: Hito 4 Order 7 scene-component-authoring)
