...
*Last updated: v0.71.0 — 2026-07-19 (Hito 4 Order 6 code-aware-ai PR2/3 COMPLETE (PR #84): frontend types/ai.ts + services/ai-context.ts (multi-source orchestrator, priority-based greedy fill under shared token budget, mirrors Rust ContextBuilder per ADR-0015); ProposeRequest extended with 4 optional fields (backward compat); fetchPropose accepts extraContext; useAIAssistant fetches source files + subscribes to hot-reload-source events; BONUS fix: restored get_command_bus_ptr/len + get_event_bus_ptr/len wasm exports lost in HIGH-1 phase 4; 52 ai-proxy + 417 editor-core tests passing; PR3 UI (context debug view + 4 E2E) pending; PR1 backend complete from v0.70.0)
