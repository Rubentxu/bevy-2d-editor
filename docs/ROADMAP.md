...
*Last updated: v0.82.0 — 2026-07-24 (`v0.82-p2-floating-multi-select` COMPLETE per ADR-0025. Two stacked-to-main PRs: #117 (PR1 floating panels + DockPrefs v3), #118 (PR2 inspector multi-select + `SetComponentFieldOnMultiple`). Merge commits `abde2cb` / `364cc32`. Bundle delta: 352.70 KB → 346.18 KB gzipped `index-*.js` (cumulative +3.48 KB above 350 KB target — within ADR-0025 estimate; chunk-splitting refactor still deferred). Multi-select Playwright: 6/6; floating-panel Playwright: 5/5. Rust 536/536 editor-core tests (7 new). 0 new clippy errors / 0 lint warnings / 0 tsc errors. Pre-existing clippy baseline (5 errors) unchanged. Hito 6 `defold-inspired-dock-polish` and Hito 7 `scene-component-authoring-ux` (v0.78.0) remain the upstream transports; two `test.skip()` blocks in `scene-component-authoring.spec.ts` for the OPFS catalog-persistence flake remain pre-existing debt per ADR-0017. Next cycle candidates: tab groups (#7), asset browser thumbnails (#8), welcome tour step-through (#9), chunk-splitting refactor — see `ROADMAP_addendum_v0.81.md`.)