...
*Last updated: v0.76.0 — 2026-07-19 (Hito 4 final cleanup COMPLETE (PR #89 v0.76.0): fixed DEFAULT_SCENE_JSON version type (1 -> \"1\") + ADR-0017 documenting the Bevy 0.19 query conflict (B0001) that still blocks 8 E2E tests; the previous \"Vite optimizeDeps bundle-cache race\" diagnosis was a red herring; the actual root cause is a multi-day Bevy 0.19 engine debugging effort; Hito 4 Orders 1-7 complete; 8 pre-existing E2E tests remain blocked by Issue 2 (Bevy engine) — workarounds documented in ADR-0017; 423 editor-core + 53 ai-proxy tests passing; Hito 4 final state: 3 features complete (code-aware-ai + scene-component-authoring + JSON fix), 1 known blocker with full documentation)
