...
*Last updated: v0.73.0 — 2026-07-19 (Hito 4 Order 7 scene-component-authoring PR1/3 COMPLETE (PR #86): backend foundation — SchemaKind enum (Simple/SceneComponent) + 3 #[serde(default)] fields on ComponentSchema (kind, bound_scene_asset_ref, auto_spawn) + FieldType extensions (ComponentRef, Enum untagged for backward compat) + BsnIrNode.kind (NodeKind enum) + 3 new Command variants (CreateSceneComponent, UpdateSceneComponentFields, BindSceneToSchema) for full undo/redo + CommandError::Unsupported variant + 3 new WASM exports (create_scene_component, bind_scene_to_schema, list_scene_component_schemas) + SchemaError::MissingBoundSceneAsset + validation; 423 editor-core tests passing (was 417 + 6 new); 5x stress stable; bsn_codegen @TypeName emission + bsn_import @ token parsing + specialized apply handlers deferred to PR2/PR3; Hito 4 Order 6 complete from v0.72.0)
