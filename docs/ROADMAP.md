...
*Last updated: v0.83.0 — 2026-07-24 (`v0.82-p3-asset-thumbnails` COMPLETE per ADR-0026. Single stacked-to-main PR: #119 (inline 64×64 asset-browser preview thumbnails + LRU Blob URL cache + `IntersectionObserver` lazy load + `opfs_save_binary` / `opfs_load_binary` engine-bridge wiring). Merge commit `9da7683`. Tag `v0.83.0`. Bundle delta: +1.1 KB gzipped over v0.82.0 (cumulative +4.58 KB above 350 KB target — within ADR-0026 estimate; chunk-splitting refactor still deferred). Playwright `asset-thumbnails.spec.ts`: 4/4. Rust 583/583 editor-core tests (3 new). 0 new clippy errors / 0 lint warnings / 0 tsc errors. Pre-existing clippy baseline (5 errors) and pre-existing `project-asset-browser.spec.ts` S1/S11/S12 failures (assetId type-mismatch, unrelated to this cycle) carried. Hito 6 `defold-inspired-dock-polish` (v0.74.0) and Hito 7 `scene-component-authoring-ux` (v0.78.0) remain the upstream transports; two `test.skip()` blocks in `scene-component-authoring.spec.ts` for the OPFS catalog-persistence flake remain pre-existing debt per ADR-0017. Next cycle candidates: tab groups (#7), welcome tour step-through (#9), chunk-splitting refactor — see `ROADMAP_addendum_v0.81.md`.)