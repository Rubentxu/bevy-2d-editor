...
| `smoke-budget-cargo-conformance-restore` (Block I)  | 🔄 IN PROGRESS (2026-09-08), cycle `p-28fce7028ac3c497/smoke-budget-cargo-conformance-restore`, sequence 176 | Restore `cargo check --workspace --tests` green (3 test files: `state_migration_pr2`, `runtime_delta_wiring`, `state_unified` missing `runtime_*_mut` delegations added in H2.5 Block A). Trim Playwright smoke cohort to `@smoke`-tagged specs only (39 tests in 4 files, was 60+ tests). Smoke 60 s budget restored. ADR-0064 ratified. **Pre-existing failures to follow up:** 3 logic_evaluator integration tests (`test_submit_and_drain`, `test_entity_bits_preserved_in_bus`, `test_end_to_end_actuator_pipeline`) fail because `submit_actuator_output` requires session install (no FALLBACK for actuator_outputs in Block A2 — separate cycle needed). |
