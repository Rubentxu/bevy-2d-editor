/**
 * perf-10k-entities.spec.ts — G6 P1: 10k entity scene roundtrip.
 *
 * Per `docs/sddk/g6-performance-corpus/specification.md` §3.1, this
 * measures the wall-clock time to:
 *
 *   1. Pre-build a 10k-entity project (outside the budget).
...
const SPEC: PerfBudgetSpec = {
  name: "perf-10k-entities",
  softMs: 8_000,
  hardMs: 20_000,
};

const TARGET_ENTITIES = 10_000;

test.describe(
  "perf — 10k entity scene roundtrip",
  { tag: ["@performance", "@full"] },
  () => {
    test("reload of a 10k-entity project hydrates within budget", async ({
      page,
    }) => {
      // Pre-build: write the 10k-entity project file outside the budget.
      // We bypass the UI to avoid 10k clicks; we use the same
      ...
      // Optional correctness assertion: the roundtrip preserves 10k entities.
      ...
      console.log(`perf: 10k-entities hydrate took ${ms} ms`);
    });
  },
);
