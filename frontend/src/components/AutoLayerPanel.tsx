...
export const AutoLayerPanel: React.FC<Props> = ({ layer, assetRef, onRegenerate }) => {
  ...
  const handleOutputTileChange = useCallback((tilesetId: string, localIndex: number) => {
    const newOutput: TileRefPayload[] = [{ tileset_id: tilesetId, local_index: localIndex }];
    const updated = [...rules];
  ...
  const handleChanceChange = useCallback((value: number) => {
    const updated = [...rules];
  ...
  const selectedTileset = tilesets.find((ts) => ts.id === (currentOutput?.tileset_id ?? layer.tileset_id));
  const tilesetOptions = selectedTileset
    ? Array.from({ length: Math.floor((selectedTileset.columns * selectedTileset.tile_height * selectedTileset.spacing) / selectedTileset.tile_width) ?? 256 }, (_, i) => i)
    : [];
    ...
    <div className="auto-layer-panel">
      ...
      {stale && (
        <div className="stale-banner">
          <span>Layer is stale — source tiles changed. Regenerate to update.</span>
          ...
          ([0, 1, 2] as const).map((col) => {
            ...
            return (
              <div
                ...
                title={isCenter ? 'Center (ignored)' : CELL_LABELS[`${row},${col}`]}
              >
                {isCenter ? (
                  ...
                  <select
                    value={value}
                    onChange={(e) => handleCellChange(row, col, e.target.value as PatternCell)}
                  >
      ...
      <div className="tile-picker">
        ...
        {selectedTileset && (
          <select
            ...
            onChange={(e) => handleOutputTileChange(selectedTileset.id, parseInt(e.target.value, 10))}
          >
      ...
      <div className="chance-slider">
        <label>
          Chance: {currentRule.chance !== undefined ? `${Math.round((currentRule.chance ?? 0) * 100)}%` : 'Always'}
        </label>
        <input
          type="range"
          min="0"
          max="100"
          value={currentRule.chance !== undefined ? Math.round((currentRule.chance ?? 0) * 100) : 100}
      ...
      <div className="rule-actions">
        ...
        <button onClick={handleRemoveRule} disabled={rules.length <= 1}>Remove Rule</button>
        <button onClick={handleRegenerate} disabled={regenerating} className="regen-btn">
          {regenerating ? 'Regenerating…' : 'Regenerate'}
        </button>
      </div>
    </div>
