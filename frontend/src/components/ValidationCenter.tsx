...
import { getValidationIssues, ValidationIssue } from "../services/validation-center";
...
function SeverityIcon({ severity }: { severity: "error" | "warning" | "info" }) {
  if (severity === "error") return <span className="vc-severity-icon vc-severity-error" title="Error">⛔</span>;
  if (severity === "warning") return <span className="vc-severity-icon vc-severity-warning" title="Warning">⚠️</span>;
  return <span className="vc-severity-icon vc-severity-info" title="Info">ℹ️</span>;
}

function CategoryBadge({ category }: { category: ValidationIssue["category"] }) {
  return <span className={`vc-category-badge vc-category-${category}`}>{category}</span>;
}

function IssueItem({ issue }: { issue: ValidationIssue }) {
  return (
    <li className={`vc-issue vc-issue-${issue.severity}`} data-testid={`vc-issue-${issue.id}`}>
...
function IssueGroup({ label, issues, severity }: { label: string; issues: ValidationIssue[]; severity: "error" | "warning" | "info" }) {
  ...
  return (
    <section className={`vc-group vc-group-${severity}`} data-testid={`vc-group-${severity}`}>
    ...
    <aside className="validation-center" data-testid="validation-center">
      <header className="vc-header">
        ...
        <div className="vc-header-actions">
          ...
          <button className="vc-close-btn" onClick={onClose} title="Close" data-testid="vc-close-btn">
            ✕
          </button>
        </div>
      </header>
      ...
      <div className="vc-body">
        {loading && <p className="vc-loading" data-testid="vc-loading">Loading...</p>}
        {error && <p className="vc-error" data-testid="vc-error">{error}</p>}
        ...
        {!loading && !error && totalCount > 0 && (
          <>
            <p className="vc-summary" data-testid="vc-summary">
              {errorCount > 0 && <span className="vc-summary-errors">{errorCount} error{errorCount !== 1 ? "s" : ""}</span>}
              {errorCount > 0 && warningCount > 0 && ", "}
              {warningCount > 0 && <span className="vc-summary-warnings">{warningCount} warning{warningCount !== 1 ? "s" : ""}</span>}
            </p>
            <IssueGroup label="Errors" issues={grouped.error} severity="error" />
            <IssueGroup label="Warnings" issues={grouped.warning} severity="warning" />
